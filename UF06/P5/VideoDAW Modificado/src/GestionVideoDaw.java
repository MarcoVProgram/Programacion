...
public class GestionVideoDaw {
    public static void main(String[] args) {
        ...
        GeneroPeli generoPeliculaSeleccion = GeneroPeli.ROMANCE; //Inicializamos para no tener errores
        String devolucion;
        int option;
        boolean diaValido;
        ...
        do {
            ...
            switch (option) {
                ...
                case 3:
                    ...
                    //Insercion de Titulo
                    MyUtils.print("Introduce el Titulo de la Pelicula:");
                    tituloPelicula = sc.nextLine();

                    //Insercion del Genero
                    MyUtils.print("Seleccione el Genero de la Pelicula");
                    for (GeneroPeli genero : GeneroPeli.values()) {
                        MyUtils.print(genero.ordinal() + " " + genero.name());
                    }

                    MyUtils.print("Introduce el Indice:");
                    generoPelicula = MyUtils.inputRequestLoop("Seleccione un indice valido (numero a la izquierda):",generoForm);
                    for (GeneroPeli genero : GeneroPeli.values()) {
                        if (genero.ordinal() == Integer.parseInt(generoPelicula)) {
                            generoPeliculaSeleccion = genero;
                        }
                    }

                    //Creacion de la Pelicula una vez hecha
                    peliculaSelected = new Pelicula(tituloPelicula, generoPeliculaSeleccion);
                ...
                case 4:
                    ...
                    do {
                        fechaNMes = MyUtils.inputRequestLoop("Mes introducido no valido, introduce uno de 2 digitos:",mesForm);
                        if (Integer.parseInt(fechaNMes) < 1 || Integer.parseInt(fechaNMes) > 12) {
                            MyUtils.print("Rango de mes no Valido, tiene que ser del 1 al 12, intentelo de nuevo:");
                        }
                    } while (Integer.parseInt(fechaNMes) < 1 || Integer.parseInt(fechaNMes) > 12);

                    //Insercion Dia
                    MyUtils.print("Dia (day) de nacimiento:");
                    do {
                        fechaNDia = MyUtils.inputRequestLoop("Dia introducido no valido, introduce uno de 2 digitos:",diaForm);
                        switch (Integer.parseInt(fechaNMes)) {
                            ...
                            case 2:
                                ...
                                break;
                            default:
                                MyUtils.print("Algo fue mal al comprobar dia, notifica a un desarrollador");
                                diaValido = true;
                                break;
                        }
                        if (diaValido) {
                            MyUtils.print("Rango de dia no Valido, tiene que ser dependiendo de los dias del mes, intentelo de nuevo:");
                        }
                    } while (diaValido);
    ...
    public static void registroDeVideoDaw(ColeccionVideoDaw coleccion) {
        ...
        //CIF
        MyUtils.print("Introduce el CIF (Codigo de Identificacion Fiscal):");
        CIF = MyUtils.inputRequestLoop("El dato introducido no es un CIF valido. Usa una letra, 7 numeros y un caracter alfanumerico (letra o numero):",CIFform);
